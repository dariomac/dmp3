...
const { customReplacer } = require('./printer');
...
async function _setRandomState(playState, items, playingStat, dirname, goDeep) {
  ...
  if (playingStat.isDirectory()) {
    ...
    const playingIdx = _.random(0, items.length-1);
    playState.playing.path = items[playingIdx];
    
    const res = await _setState(playState);
    if (res) {
      console.log('<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<');
      console.log('Playing:', res.playing.path);
      console.log('Next: ', res.next.path);
      console.log('FullyPlayed:', JSON.stringify(res.fullyPlayedDirs, customReplacer, 2));
    }
    return res;
  }
  else {
...
async function _setState(playState) {
  ...
  console.log('Next: ', playState.next.path);
  console.log('FullyPlayed: ', JSON.stringify(playState.fullyPlayedDirs, customReplacer, 2));
