...
export function printSong (playState) {
  try {
...
export function printBye () {
  process.stdout.write(chalk.green(`\nBye !!!\n`));
}

export function printEOS () {
  process.stdout.write(chalk.green(`\nThis is the end.\nNo more MP3s to play\n`));
}

export function printDetails(playState) {
...
export function expand (content) {
  logUpdate(content);
  expanded = true;
}
...
export function printLyrics(song) {
  if (expanded) {
    collapse();
  }
  else {
