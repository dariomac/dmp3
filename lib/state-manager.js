...
class StateManager {
  ...
  async #getPlayableAudioFiles(dirname) {
    let items = _
      .filter((await this.#readdir(dirname)), function (item) {
  ...
  async #collectRandirCandidates(playState, dirPath) {
    let candidates = [];

    const audioFiles = await this.#getPlayableAudioFiles(dirPath);
    if (audioFiles.length > 0 && !playState.fullyPlayedDirs.find(dirPath)) {
      candidates.push(dirPath);
    }

    const entries = await this.#readdir(dirPath, { withFileTypes: true });
    const subdirs = _
      .filter(entries, function (item) {
        return item.isDirectory();
      })
      .map(function (item) {
    ...
    for (const subdir of subdirs) {
      candidates = candidates.concat(await this.#collectRandirCandidates(playState, subdir));
    }

    return candidates;
  }

  async #setRandirState(playState, playingStat) {
    if (playingStat.isDirectory()) {
      const candidates = await this.#collectRandirCandidates(playState, playState.topLevelDir);

      if (candidates.length === 0) {
        return null;
      }

      const dirIdx = await randomInt(0, candidates.length - 1);
      const chosenDir = candidates[dirIdx];
      const items = await this.#getPlayableAudioFiles(chosenDir);

      playState.playing.path = items[0];
    ...
    const currentDir = path.dirname(playState.playing.path);
    const items = await this.#getPlayableAudioFiles(currentDir);
    const playingIdx = _.findIndex(items, function (item) {
    ...
    if (nextIdx < items.length) {
      ...
      this.#pushDirAsPlayed(playState, currentDir);
      playState.next = createSong(playState.topLevelDir);
    }
  ...
  async #setSortedState(playState, items, playingStat, dirname) {
    ...
    if (nextIdx < items.length) {
      ...
      this.#pushDirAsPlayed(playState, dirname);
      const nextState = this.#changeToPriorDirectory(playState, dirname);
      if (nextState) {
        playState = nextState;
      } else {
        playState.next = {};
  ...
  async setState(playState) {
    ...
    if (items.length === 0) {
      ...
      if (playState.mode === 'randir') {
        playState = await this.#setRandirState(playState, playingStat);
      } else if (playState.mode === 'random') {
