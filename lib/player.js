...
async function _setRandomState(playState) {
  ...
  if (playingStat.isDirectory()) {
    ...
    //console.log(playState.playing.path)
...
async function _setSortedState(playState) {
  playState.playing.path = path.resolve(playState.playing.path);
...
async function setState(playState) {
  if (playState.mode === 'random') {
    return await _setRandomState(playState);
  }
  else {
    return await _setSortedState(playState);
  }
}
...
async function play () {
  ...
  if (_playState.playing.pid === 0) {
    await meta.setDuration(_playState.playing);
    _playState.playing = _osPlayer.play(_playState.playing);
    await meta.setTags(_playState.playing);
